  ...
  <div class="p-6 max-w-2xl">
      ...
      <div class="space-y-3">
        <div class="flex items-center justify-between">
          <div>
            ...
            <div class="text-xs font-mono text-blades-muted">Download all data as a full JSON backup</div>
          </div>
        ...
        <div class="flex items-start justify-between gap-4">
          <div class="min-w-0">
            <div class="text-sm font-sans text-blades-gold">Export for GitHub Pages</div>
            <div class="text-xs font-mono text-blades-muted leading-relaxed mt-0.5">
              Downloads <span class="text-blades-text">campaign-data.json</span> with your public data.
              Save it to <span class="text-blades-text">public/campaign-data.json</span> in your repo, commit, and push.
              Images are included as data URLs — your static site will show everything.
            </div>
          </div>
          <button class="blades-btn-gold text-xs flex-shrink-0 mt-0.5" @click="exportForGhPages">↑ Export</button>
        </div>
        <div class="blades-divider" />
        <div class="flex items-center justify-between">
          <div>
    ...
    <div class="blades-card p-4 mb-6 border-blades-sage/20">
      ...
      <div class="space-y-3">
        ...
        <div class="flex items-center justify-between">
          <div>
            <div class="text-sm font-sans text-blades-text">Seed Aurelion NPCs</div>
            <div class="text-xs font-mono text-blades-muted">Add the 14 named world characters (faction heads, notable figures) — replaces any with the same IDs</div>
          </div>
          <button class="blades-btn-ghost text-xs" @click="seedNpcs">Seed NPCs</button>
        </div>
      </div>
    </div>
...
function exportAll() {
  const data = {
    ...
    journal: journalStore.exportSnapshot(),
  }
...
function importAll(e: Event) {
  ...
  reader.onload = () => {
    try {
      ...
      if (data.journal)    { localStorage.setItem('ttdb_journal',    JSON.stringify(data.journal));    journalStore.load() }
...
function clearAll() {
  ...
  ['ttdb_characters', 'ttdb_locations', 'ttdb_factions', 'ttdb_items', 'ttdb_crew', 'ttdb_journal'].forEach(k => localStorage.removeItem(k))
  location.reload()
}
...
function seedNpcs() {
  if (!confirm('Add/replace the 14 Aurelion world NPCs? Existing NPCs with different IDs will not be affected.')) return
  charStore.seedAurelionNpcs()
}
</script>
